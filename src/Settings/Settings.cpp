...
const char DEFAULT_WALLET_FILE_NAME[] = "B2B.wallet";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("b2bwallet.cfg"));
...
const Style& Settings::getCurrentStyle() const {
  QReadLocker lock(&m_lock);
  QString theme = m_settings.contains(OPTION_WALLET_THEME) ? m_settings.value(OPTION_WALLET_THEME).toString() : "b2b";
...
QString Settings::getCurrentTheme() const {
  QReadLocker lock(&m_lock);
  return m_settings.contains(OPTION_WALLET_THEME) ? m_settings.value(OPTION_WALLET_THEME).toString() : "b2b";
}
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("b2bwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("b2bwallet.desktop");
  ...
  res = autorunSettings.contains("B2BcoinWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("B2BcoinWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "org.b2b.b2bwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=B2Bcoin QT GUI Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("B2BcoinWallet", appPath);
    } else {
      autorunSettings.remove("B2BcoinWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\b2b", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:b2b");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\b2b\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\b2b\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("b2bwallet.cfg"));
