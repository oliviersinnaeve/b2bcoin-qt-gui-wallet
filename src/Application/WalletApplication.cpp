...
const char COIN_URI_SCHEME_NAME[] = "b2bcoin";
...
WalletApplication::WalletApplication(int& _argc, char** _argv) : QApplication(_argc, _argv), m_lockFile(nullptr),
  ...
  setApplicationName("B2BcoinWallet");
...
bool WalletApplication::init() {
  ...
  m_lockFile = new QLockFile(Settings::instance().getDataDir().absoluteFilePath("b2bcoinwallet.lock"));
  QUrl paymentUrl = QUrl::fromUserInput(arguments().last());
  if (paymentUrl.scheme() != COIN_URI_SCHEME_NAME) {
    paymentUrl = QUrl();
  }
  ...
  if (!m_lockFile->tryLock()) {
    WalletLogger::warning(tr("[Application] B2Bcoin QT GUI wallet is already running"));
    if (!paymentUrl.isValid()) {
      QMessageBox::warning(nullptr, QObject::tr("Fail"), "B2Bcoin QT GUI wallet is already running");
    }
...
bool WalletApplication::initCryptoNoteAdapter() {
  for (;;) {
    if (m_splash != nullptr) {
      m_splash->show();
      m_splash->showMessage(QObject::tr("B2Bcoin QT GUI Wallet v2.0.1 - Loading..."), Qt::AlignLeft | Qt::AlignBottom, Qt::white);
    ...
    case INodeAdapter::INIT_DB_IN_USAGE: {
      ...
      "If you have b2bcoind with non-default RPC port, you should terminate it and relaunch B2Bcoin QT GUI wallet\n"
      "or\n"
      "Set the Local daemon required port in B2Bcoin QT GUI wallet Menu/Preferences/Connection settings."));
	  if (m_splash != nullptr)
		  m_splash->hide();
      dlg.exec();
      return false;
